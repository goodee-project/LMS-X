...
public class TeacherQuestionController {
	...
	// 질문 조회
	@GetMapping("/auth/teacher/lecture/{lectureNo}/question/questionOne/{questionNo}/{currentPage}")
	public String questionOne(Model model,
		...
		// 댓글 목록
		List<QuestionComment> questionComment = teacherQuestionCommentService.getTeacherQuestionCommentListByPage(questionNo, beginRow, rowPerPage);
		
		// 페이징 코드
		// 현재 페이지 수
		int totalCount = teacherQuestionCommentService.getCountTeacherQuestionComment(questionNo);
		// 마지막 페이지
		int lastPage = totalCount / rowPerPage;
		// 10 미만의 개수의 데이터가 있는 페이지 표시
		if (totalCount % rowPerPage != 0) {
